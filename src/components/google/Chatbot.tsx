...
import { getServiceDental } from "../../service/apiService";
...
const Chatbot: React.FC = () => {
  ...
  >([
    {
      role: "system",
      content:
        "Bạn là một chuyên gia về spa massage trị liệu của công ty SPA Royal.Câu trả lời ngắn gọn nhưng đầy đủ thông tin cho người dùng.",
    },
  ]);
  ...
  const checkDentalRelated = (message: string) => {
    const keywords = [
      // Chủ đề chung
      "nha khoa",
      "răng",
  ...
  const fetchSpaServices = useCallback(async () => {
    try {
      const services: ServiceFull[] = await getServiceDental();
  ...
  const sendMessage = useCallback(async () => {
    ...
    if (
      ["danh sách dịch vụ", "tất cả dịch vụ"].some((keyword) =>
      ...
      setTimeout(() => {
        setMessages((prev) => [
          ...prev,
          {
            role: "assistant",
            content: "",
            jsxContent: serviceDetailsElements,
          },
        ]);
      }, 2000);
      setLoading(false);
    ...
    const matchingServices = spaServices.filter(
      (service) =>
    ...
    if (!checkDentalRelated(input)) {
      setMessages((prev) => [
        ...prev,
        {
          role: "assistant",
          content:
            "⚠ Xin lỗi, tôi chỉ hỗ trợ các câu hỏi liên quan đến lĩnh vực **nha khoa** (chăm sóc và điều trị răng miệng).",
        },
      ]);
      setLoading(false);
      return;
  ...
  return (
    <div className="relative w-full min-h-screen px-4 sm:px-8 lg:px-16 flex flex-col sm:flex-row items-center justify-center bg-gradient-to-br from-purple-900 via-blue-900 to-black">
      <motion.div className="w-full sm:w-[90vw] mx-auto p-6 bg-white/20 rounded-xl shadow-lg border border-gray-200 absolute top-1/2 left-1/2 transform -translate-x-1/2 -translate-y-1/2 dark:bg-gray-800 dark:border-gray-500">
        ...
        <div
          className="overflow-y-auto p-4 rounded-lg space-y-3 dark:bg-gray-800"
        ...
        >
          {messages.slice(1).map((msg, idx) => (
            ...
            >
              {msg.role === "assistant" && (
                <FaRobot className="text-blue-500 mt-2 mr-2" />
              )}
              <div
              ...
              {msg.role === "user" && (
                <FaUser className="text-gray-600 mt-2 ml-2" />
              )}
            </div>
          ))}
