...
interface ServiceSpaEditModalProps {
  ...
  handleUpdateService: (
    id: number,
    dataToUpdate: ServiceDentalFormUpdate
  ) => void;
...
const ServiceSpaEditModal: React.FC<ServiceSpaEditModalProps> = ({
  isEditDialogOpen,
          ...
          <DialogContent className="bg-gray-50 p-6 mt-4">
            <form
              onSubmit={(e) => {
                ...
                const updatedServiceData: ServiceDentalFormUpdate = {
            ...
            >
              ...
              <div className="sm:grid sm:grid-cols-2 sm:gap-4 flex flex-col gap-y-2">
                ...
                <div>
                  <label className="block sm:text-sm text-[12px] font-medium text-gray-700 mb-1">
                    {" "}
                    {/* Giữ class label cũ cho nhất quán nếu muốn */}
                  ...
                  >
                    ...
                    {/* Thêm disabled cho option mặc định */}
                    {Array.from({ length: (120 - 30) / 5 + 1 }, (_, index) => {
                      const value = 30 + index * 5;
                      return (
                        <option key={value} value={value.toString()}>
                          {" "}
                          {/* value của option cũng nên là string */}
                          {value} phút
                        </option>
                      );
                    })}
