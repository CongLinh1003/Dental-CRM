...
const menuItems: MenuItem[] = [
  ...
  // },
  {
    ...
    id: "labo",
    subItems: [{ name: "Danh sách Labo", page: "danhsachLabo", index: 30 }],
  },
  {
...
const MenuDrawer: React.FC = () => {
  ...
  return (
    <div className="flex sm:w-screen dark:bg-gray-800 bg-white text-gray-900 dark:text-white md:pb-10">
      <div
        className={`flex flex-col ${drawerOpen ? "sm:w-96 w-screen h-screen" : "relative w-0"} transition-all duration-300`}
      >
        <div className="flex items-center justify-end px-2 py-2 ">
          <button
            onClick={toggleDrawer}
            className={`p-4 rounded-full ${drawerOpen ? "" : "absolute w-[70px] pl-8 top-0 left-[-20px]"} bg-white dark:bg-black hover:bg-gray-300`}
          >
            {drawerOpen ? <Menu /> : <ChevronRight />}
          </button>
        </div>
        <div
          className={`overflow-y-auto ${drawerOpen ? "sm:h-[80vh] h-[60vh] p-4" : "overflow-hidden"} sm:h-[70vh] h-[60vh] max-h-full flex flex-col text-[18px]`}
        >
          {menuItems.map((menuItem) => {
            ...
            return (
              <div key={menuItem.id} className="text-[14px] sm:text-[16px]">
                ...
                >
                  <IconComponent />
                  <span className={`ml-3 ${drawerOpen ? "block" : "hidden"}`}>
                    {menuItem.name}
                  </span>
                </button>
                {isOpen && (
                  <div
                    className="ml-6 p-4 bg-gray-100 border-l-2 border-blue-400 rounded-r-md"
                    ...
                    {menuItem.subItems?.map((subItem) => {
                      ...
                      return (
                        <button
                          ...
                          onClick={() =>
                            handleNavigation(subItem.page, subItem.index)
                          }
                        >
                          {subItem.name}
                        </button>
