...
const mockAssignments: AssignmentStaffData[] = [
  ...
  },
  {
    ...
    department: {
      departmentId: 2,
      departmentName: "Phòng phẫu thuật",
      description: "Phòng phẫu thuật chuyên sâu",
      createdAt: "2024-01-01T00:00:00Z",
      updatedAt: "2024-01-01T00:00:00Z",
      status: "ACTIVE",
    },
  ...
  },
  {
    ...
    department: {
      departmentId: 3,
      departmentName: "Phòng X-quang",
      description: "Phòng chụp X-quang",
      createdAt: "2024-01-01T00:00:00Z",
      updatedAt: "2024-01-01T00:00:00Z",
      status: "ACTIVE",
    },
...
// Kiểu sự kiện sử dụng cho react-big-calendar (dùng generic object để tránh xung đột type nội bộ)
interface CalendarEventBase {
  title: string;
  start: Date;
  end: Date;
  allDay?: boolean;
  resource: AssignmentStaffData; // resource luôn tồn tại
}

const formatAssignmentsForCalendar = (
...
const AssignmentSchedule: React.FC = () => {
  ...
  const [selectedEvent, setSelectedEvent] =
    useState<AssignmentStaffData | null>(null);
  ...
  const handleSelectEvent = (event: object) => {
    const ev = event as CalendarEventBase;
    setSelectedEvent(ev.resource);
    setShowDetailModal(true);
  };

  const handleEventDrop = (args: any) => {
    const evDropped = args.event as CalendarEventBase;
    const start = args.start as Date;
    const end = args.end as Date;
    if (!evDropped.resource) return;
    const updatedEvents = events.map((ev) => {
      if (ev.resource.id === evDropped.resource.id) {
        return {
          ...ev,
          start,
          end,
          resource: { ...ev.resource, assignedDate: start.toISOString() },
        };
      }
      return ev;
    });
    ...
    toast.success(
      `Đã dời lịch hẹn của ${evDropped.resource.appointment.patientName} đến ${moment(start).format("HH:mm DD/MM/YYYY")}`
    );
  };

  const handleToggleImportant = (eventId: number) => {
    const updatedEvents = events.map((ev) =>
      ev.resource.id === eventId
        ? {
            ...ev,
            resource: {
              ...ev.resource,
    ...
    setEvents(updatedEvents);
    const nowImportant = updatedEvents.find((e) => e.resource.id === eventId)
      ?.resource.isImportant;
    if (nowImportant) toast.info("Đã gắn tag quan trọng!");
    else toast.warn("Đã bỏ tag quan trọng.");
  };

  const EventComponent = ({ event }: EventProps<object>) => {
    const ev = event as CalendarEventBase;
  ...
  const eventPropGetter = (event: object) => {
    const ev = event as CalendarEventBase;
    const backgroundColor = ev.resource.isImportant ? "#ffdddd" : "#a8e6cf";
    const style = {
      backgroundColor,
      borderRadius: "5px",
      opacity: 0.8,
      color: "black",
      border: `1px solid ${ev.resource.isImportant ? "#f44336" : "#62a87c"}`,
    };
    return { style };
  };
  ...
  return (
    ...
    >
      <ToastContainer />
      <p className="sm:text-2xl text-lg font-bold mb-6">
          ...
          <div className="bg-white rounded-lg shadow-xl max-w-lg w-full p-6">
            ...
            <div className="space-y-3 text-sm">
              ...
              <p className="text-gray-700">
                ...
                {selectedEvent.appointment.service.serviceName}
              </p>
              <p className="text-gray-700">
                <strong>Nhân viên:</strong> {selectedEvent.staff.name}
              </p>
              <p className="text-gray-700">
                <strong>Phòng ban:</strong>{" "}
                {selectedEvent.department.departmentName}
              </p>
              <p className="text-gray-700">
              ...
              <p className="text-gray-700">
                ...
                {moment(selectedEvent.assignedDate).format("HH:mm DD/MM/YYYY")}
              </p>
              <p className="text-gray-700">
                <strong>Trạng thái:</strong>{" "}
                <span
                  className={`font-semibold ${selectedEvent.appointment.status === "CONFIRMED" ? "text-green-600" : "text-yellow-600"}`}
                >
              ...
              <p className="flex items-center text-gray-700">
                ...
                <span
                  className={`ml-2 px-2 py-1 rounded-full text-xs font-bold ${selectedEvent.isImportant ? "bg-red-500 text-white" : "bg-gray-200 text-gray-600"}`}
                >
